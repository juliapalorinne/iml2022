...
loocvT <- function(
    ...
    model = lm,
    train = function(data) model(formula, data = data),
    pred = function(model, data) predict(model, newdata = data, type="class")) {
  yhat <- NULL
  for (i in 1:nrow(data)) {
    mod <- train(data[-i, ])
    if (is.null(yhat)) {
...
npf.168F <- data.frame(npf.168, classF)

idx1 <- sample(nrow(npf.fact), nrow(npf.fact)/2)
npf.fact.train <- npf.fact[idx1, ]
npf.fact.test <- npf.fact[-idx1, ]
idx2 <- sample(nrow(npf.168F), nrow(npf.168F)/2)
npf.168F.train <- npf.168F[idx2, ]
npf.168F.test <- npf.168F[-idx2, ]

tree.train.fact <- tree(class2 ~ ., npf.fact.train)
summary(tree.train.fact)
tree.train.168.fact <- tree(classF ~ . -class2, npf.168.factF)
...
tree.pred.168.fact <- predict(prune.train.168.fact, npf.168F.test)
LOOCV.tree = loocv(prune.train.fact, data = npf.fact.test)
LOOCV.168.tree = loocv(prune.train.168.fact, data = npf.168F.test)
CV.tree <- cv(prune.train.fact, data = npf.fact.test)
CV.168.tree = cv(prune.train.168.fact, data = npf.168F.test)
...
npfF <- npfF[, -(ncol(npf))]
npf.168F <- npf.168F[, -(ncol(npf.168))]

idx1 <- sample(nrow(npfF), nrow(npfF)/2)
npfF.train <- npfF[idx1, ]
npfF.test <- npfF[-idx1, ]
idx2 <- sample(nrow(npf.168F), nrow(npf.168F)/2)
npf.168F.train <- npf.168F[idx2, ]
npf.168F.test <- npf.168F[-idx2, ]

tree.train.fact <- tree(classF ~ ., npfF.train)
tree.train.168.fact <- tree(classF ~ ., npf.168F.train)

prune.train.fact <- prune.misclass(tree.train.fact, best = 8)
prune.train.168.fact <- prune.misclass(tree.train.168.fact, best = 8)

tree.pred.fact <- predict(prune.train.fact, npfF.test, type = "class")
tree.pred.168.fact <- predict(prune.train.168.fact, npf.168F.test, type = "class")
LOOCV.tree = loocvT(prune.train.fact, data = npfF.test)
LOOCV.168.tree = loocvT(prune.train.168.fact, data = npf.168F.test)
CV.tree <- cvT(prune.train.fact, data = npfF.test)
CV.168.tree = cvT(prune.train.168.fact, data = npf.168F.test)

acc.tree <- data.frame(original = mean(tree.pred.fact == npfF.test$classF),
                       CV = mean(CV.tree == npfF.test$classF),
                       LOOCV = mean(LOOCV.tree == npfF.test$classF))
acc.tree.168 <- data.frame(original = mean(tree.pred.168.fact == npf.168F.test$classF),
                           CV = mean(CV.168.tree == npf.168F.test$classF),
                           LOOCV = mean(LOOCV.168.tree == npf.168F.test$classF))
per.tree <- data.frame(original = perplexity(tree.pred.fact, npfF.test$classF),
                       CV = perplexity(CV.tree, npfF.test$classF),
                       LOOCV = perplexity(LOOCV.tree, npfF.test$classF))
per.tree.168 <- data.frame(original = perplexity(tree.pred.168.fact, npf.168F.test$classF),
                           CV = perplexity(CV.168.tree, npf.168F.test$classF),
                           LOOCV = perplexity(LOOCV.168.tree, npf.168F.test$classF))

kable(cbind(rbind(acc.tree, acc.tree.168), rbind(per.tree, per.tree.168))) %>%
  add_header_above(c("Accuracy" = 3, "Perplexity" = 3)) %>%
  kable_styling(latex_options = "HOLD_position")


print(tree.pred.168.fact)
print(CV.tree)
...
accuracy <- function(pred, y) mean(ifelse(pred >= 0.5, 1, 0) == y)
npf.RF.acc <- accuracy(npf.RF.pred, npf_test168$class2)
npf.RF.per <- perplexity(npf.RF.pred, npf_test168$class2)

LOOCV.RF.acc = accuracy(loocv(npf.RF.mod, data=npf_test168), npf_test168$class2)
LOOCV.RF.per <- perplexity(loocv(npf.RF.mod, data=npf_test168), npf_test168$class2)


kable(cbind(c(npf.RF.acc, LOOCV.RF.acc), c(npf.RF.per, LOOCV.RF.per))) %>%
  add_header_above(c("Accuracy", "Perplexity") )%>%
  kable_styling(latex_options = "HOLD_position")
...
## Accuracy
accuracyTree <- function(pred, y) mean(ifelse((pred-1) >= 0.5, 1, 0) == y)

loocvRF <- function(
    data,
    train = function(data) randomForest(class2 ~ ., data = data, mtry = 12, importance = TRUE),
    pred = function(model, data) predict(model, newdata = data)) {
  yhat <- NULL
  for (i in 1:nrow(data)) {
    mod <- train(data[-i, ])
...
acc.tree <- data.frame(original = accuracyTree(tree.pred.fact, npf.fact.test$class2),
                       CV = accuracyTree(CV.tree, npf.fact.test$class2),
                       LOOCV = accuracyTree(LOOCV.tree, npf.fact.test$class2))
acc.tree.168 <- data.frame(original = accuracyTree(tree.pred.168.fact, npf.168.fact.test$class2),
                           CV = accuracyTree(CV.168.tree, npf.168.fact.test$class2),
                           LOOCV = accuracyTree(LOOCV.168.tree, npf.168.fact.test$class2))
