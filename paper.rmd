...
With our observed best-performing models being the the random forest model, the Lasso regression model and the linear discriminant analysis. Out of these models, we eventually chose to use the Lasso model as it was the best-performing model. The amount of information we could gain through CV calculations in R without R crashing were relatively limited, to the degree that we couldn't usually even calculate the cross-validated best choice for the tuning parameter. As a result, we chose to forego cross-validation in relation to our multinomial model.
...
We made the decision of model based on preliminary results, and though classification trees would represent an equally viable solution, we calculated those results only later on, and by that point had already calculated the results for the logistic regression model. 
