...
```{r PCA}
pcaOrig<-prcomp(npfOrig[,5:104],center = T, scale.=T)
cumsum(pcaOrig$sdev^2)/sum(pcaOrig$sdev^2)
x1<-as.matrix(npfOrig[,5:104])%*%pcaOrig$rotation[,1:12]
...
```{r lasso2}
lasso1<-cv.glmnet(x=as.matrix(npfOrig[,5:104]),y=class2, alpha=1,family="binomial", type.measure="class",nfolds=464)
lassomod1<-glmnet(x=as.matrix(npfOrig[,5:104]),y=class2, alpha=1, lambda = lasso1$lambda.min,family="binomial", type.measure="class")
preds<-c()
perps<-c()
for(i in 1:464){
  newexp<-npfOrig[-i,]
  newmod<-glmnet(x=as.matrix(newexp[,5:104]), y=newexp[,105], alpha=1,family="binomial", lambda=lasso1$lambda.min)
  newprob<-predict(newmod, newx=as.matrix(npfOrig[i,5:104]),type="response")
  if(class2[i]==1){
    newperp=log(newprob)
  } else{
